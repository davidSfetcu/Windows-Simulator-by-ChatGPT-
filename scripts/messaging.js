// scripts/messaging.js — improved notifications on send
(function(){
  ...
  function send(withWho, text){ let threads = Storage.get('messages',[]); let t = threads.find(x=>x.with===withWho); if(!t){ t={with:withWho, msgs:[]}; threads.unshift(t);} t.msgs.push({out:true, text, when:new Date().toLocaleString()}); t.last = text; Storage.set('messages',threads); load(); parent.WP.notify('Message sent', 'To: '+withWho); }
